...
// string fuzzy_matching(const string& file_name); // 模糊匹配
...
void send_message(int flag, const string& message, int sock) {
    ...
    memcpy(buffer + 2 * sizeof(int), message.c_str(), sizeof(int));
...
void send_file(stack<string>& dir_skt, int sock) {
    ...
    if (instruction == "$list") { // 如果指令要求当前目录下的所有文件
        ...
        send_message(-1, dir_cur, sock); // 发送本层目录地址
        send_file(dir_skt, sock); // 递归处理上级目录
    } else { // 如果指令指定的是某一文件
        struct stat s;
